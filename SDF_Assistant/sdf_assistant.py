...
class SDFAssistant:
    ...
    def _load_documents(self):
        ...
        try:
            ...
            logger.info(f"✅ SDF cargados {len(self.documents_cache)} documentos en {load_time:.2f}s")
    ...
    def _find_relevant_documents(self, user_message: str) -> List[str]:
        ...
        for file_name, doc_data in self.documents_cache.items():
            try:
                ...
                logger.warning(f"Error analizando relevancia de {file_name}: {e}")
                continue
        ...
        for file_name in relevant_docs:
            if file_name in self.documents_cache:
                ...
                if content and content != "None":
                    context_parts.append(f"ARCHIVO RELEVANTE: {file_name}\nTIPO: {file_type}\nCONTENIDO:\n{content}\n")
        
        full_context = "\n".join(context_parts)
        logger.info(f"Contexto relevante generado: {len(full_context)} caracteres de {len(relevant_docs)} documentos")
...
Analiza los documentos de manera inteligente y proporciona una respuesta útil y completa basada en los datos disponibles."""
            ...
            try:
                ...
                logger.info(f"✅ SDF completado en {processing_time:.2f}s usando {len(relevant_docs)} documentos relevantes")
